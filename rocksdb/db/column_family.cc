...
void ColumnFamilyHandleImpl::EnableDeferCompactions(
      const MutableCFOptions& mutable_cf_options) {
	ColumnFamilyHandle* cfh = GetColumnFamilyHandle();
	//auto* cfd = reinterpret_cast<ColumnFamilyHandleImpl*>(cfh)->cfd();
	s = db_->SetOptions(cfh, {{"defer_compactions", "true"}});
    ...
    if (imm()->NumNotFlushed() >= mutable_cf_options.max_write_buffer_number) {
      ...
      ROCKS_LOG_WARN(
	    ...
	    if (mutable_cf_options.allow_defer_compactions) {
		    EnableDeferCompactions(mutable_cf_options);
	    } else {
	      ...
	      if (mutable_cf_options.allow_defer_compactions) {
		      ...
		      // stop and speed up the slowdown.
		      uint64_t soft_limit = 
			  3.0 / 4.0 * 
			  (mutable_cf_options.hard_pending_compaction_bytes_limit
			   - mutable_cf_options.soft_pending_compaction_bytes_limit);
		      ...
		      ROCKS_LOG_WARN(
			  ioptions_.info_log,
			  "[%s] Stalling writes because of estimated pending"
			  " compaction bytes %" PRIu64 " rate %" PRIu64,
			  name_.c_str(),
			  vstorage->estimated_compaction_needed_bytes(),
			  write_controller->delayed_write_rate());
	      }
      ...
      if (vstorage->l0_delay_trigger_count() >=
	      ...
	      if (mutable_cf_options.allow_defer_compactions) {
		      ...
		      ROCKS_LOG_WARN(
			  ioptions_.info_log,
			  "[%s] Increasing compaction threads because we have %d "
			  "level-0 files ",
			  name_.c_str(), vstorage->l0_delay_trigger_count());
	      }
	      ...
	      if (mutable_cf_options.allow_defer_compactions) {
		      ...
		      // If soft pending compaction byte limit is not set, always
		      // speed up compaction.
		      write_controller_token_ =
			  write_controller->GetCompactionPressureToken();
		      if (mutable_cf_options.soft_pending_compaction_bytes_limit
			  > 0 ) {
			      ROCKS_LOG_WARN(
				  ioptions_.info_log,
